...
from utils.tag_processor import song_describer_tag_processor
from utils.multitag_processor import song_describer_multitag_processor
from utils.caption_processor import song_describer_caption_processor
...
def retrieval_target_generation(annotations, tokenizer, threshold=10):
    tag2track_matrix, track2tag, tag2track, tag_map = song_describer_tag_processor(annotations, tokenizer, threshold)
    _, track2multitag, multitag2track = song_describer_multitag_processor(annotations, tokenizer)
    _, track2caption, caption2track = song_describer_caption_processor(annotations)
    # unique_entity 
    ...
    list_of_dict = [track2idx,caption2idx,multitag2idx,tag2idx]
    list_of_dict_name = ["track2idx","caption2idx","multitag2idx","tag2idx"]
    ...
    sd_dataset = DatasetDict(song_describer_retrieval)
    
    # # if you wanna save dataset
    # for split, dataset in dataset.items():
    #     dataset.to_json(f"song_describer_retrieval_{split}.jsonl")


def load_metadata():
    annotations = pd.read_csv("../datasets/song_describer/song_describer.csv")
    annotations = annotations[annotations["is_valid_subset"].fillna(False)]
    annotations["genre"] = annotations["genre"].apply(ast.literal_eval)
    annotations["instrument"] = annotations["instrument"].apply(ast.literal_eval)
    annotations["mood/theme"] = annotations["mood/theme"].apply(ast.literal_eval)
    annotations["aspect_list"] = (
        annotations["genre"] + annotations["instrument"] + annotations["mood/theme"]
    )
    annotations["caption_id"] = annotations["caption_id"].apply(str)
    annotations["track_id"] = annotations["track_id"].apply(str)
    return annotations
